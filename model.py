...
class Global_Policy_Frontiers(NNBase):

    def __init__(self, input_shape, recurrent=False, hidden_size=512,
                 downscaling=1):
        super(Global_Policy_Frontiers, self).__init__(recurrent, hidden_size,
                                            hidden_size)
        ...
        self.linear1 = nn.Linear(out_size * 32 + 10, hidden_size)
        self.linear2 = nn.Linear(hidden_size, 256)
        self.critic_linear = nn.Linear(256, 1)
        self.orientation_emb = nn.Embedding(72, 10)
        self.train()
...
class RL_Policy_Frontiers(nn.Module):

    def __init__(self, obs_shape, action_space, model_type=0,
                 base_kwargs=None):

        super(RL_Policy_Frontiers, self).__init__()
        if base_kwargs is None:
            base_kwargs = {}

        if model_type == 0:
            self.network = Global_Policy_Frontiers(obs_shape, **base_kwargs)
        else:
            raise NotImplementedError
